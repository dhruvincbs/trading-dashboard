...
export default function ClientStrategyDetails() {
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm text-green-600 dark:text-green-400">Top 5 Winners</CardTitle>
                </CardHeader>
                <CardContent>
                      ...
                      <TableBody>
                        {details!.topWinnersByMonth[month].map((t, i) => (
                          <TableRow key={i} data-testid={`row-winner-${month}-${i}`}>
                            ...
                            <TableCell className="text-right text-green-600 dark:text-green-400">${t.profitLoss.toFixed(2)}</TableCell>
              ...
              <Card>
                <CardHeader className="pb-2">
                  <CardTitle className="text-sm text-red-600 dark:text-red-400">Top 5 Losers</CardTitle>
                </CardHeader>
                <CardContent>
                      ...
                      <TableBody>
                        {details!.topLosersByMonth[month].map((t, i) => (
                          <TableRow key={i} data-testid={`row-loser-${month}-${i}`}>
                            <TableCell className="font-medium">{t.stock}</TableCell>
                            <TableCell className="text-right text-red-600 dark:text-red-400">${t.profitLoss.toFixed(2)}</TableCell>
                            <TableCell className="text-right">{t.returnPct.toFixed(2)}%</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
